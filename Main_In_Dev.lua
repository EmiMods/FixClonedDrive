...
scriptIcon = "icon.png"
scriptPermissions = { "filesystem", "sql", "settings", "content" }
        ...
        if confirmation.Button == 2 then
            ...
            if not isTableEmpty and not ExitTriggered then
                for _, row in pairs(dbRows) do
                    --Sql.Execute("UPDATE scanpaths SET deviceid='" .. newDeviceID .. "' WHERE id=" .. row[1] .. ";")  --TODO removed for testing
                    local tempDebugText = ""
                    --TODO Where I left off, still need to test.
                    for _, v in pairs(Sql.ExecuteFetchRows("USE content; SELECT * FROM titleupdates;")) do
                        tempDebugText = tempDebugText .. v.liveDeviceId
                        Script.ShowMessageBox("DEBUG", tempDebugText, "OK")
                    end
            ...
            if updatedScanpaths then
                confirmation = Script.ShowMessageBox("Update", "Scanpaths have been updated with the selected deviceID.\n\nContinue to Title Updates..", "Continue", "Exit")
                if confirmation.Button == 2 then
                    ExitTriggered = true
                end
            end
            ...
            if not ExitTriggered then
                -- TODO Figure out if Settings.db is accessible, and trigger restart when needed
                local setting = Settings.GetSystem("TitleUpdateDevice")
                Script.ShowMessageBox("DEBUG", setting.TitleUpdateDevice.default, "OK")
            end
